...
def ExtractSDR (DynamicDirectory, HazardLevel, NumGM, NumStory):
    ...
    for i in range(NumHazardLevel):
        ...
        for j in range(NumGM[i]):
            SDRDirectory = os.path.join(DynamicDirectory, 'UniDirection_Miranda240GMs', 'EQ_%d'%(j+1), 'StoryDrifts')
            print(SDRDirectory)
...
def ExtractRDR (DynamicDirectory, HazardLevel, NumGM, NumStory):
    ...
    for i in range(NumHazardLevel):
        ...
        # Loop over all ground motions in one hazard level
        for j in range(NumGM[i]):
            RDRDirectory = os.path.join(DynamicDirectory, 'UniDirection_Miranda240GMs', 'EQ_%d'%(j+1), 'StoryDrifts')
...
#     NumGM_temp2 = np.cumsum(NumGM_temp1)
    ...
    for i in range(NumHazardLevel):
        ...
        for j in range(NumGM[i]):
            ...
            gmRecord = max(np.abs(np.loadtxt('%s.txt'%(j+1), dtype = float)))
            temp.append(gmRecord)
    
    PGA = pd.DataFrame(temp)
...
def ExtractPFA (DynamicDirectory, HazardLevel, NumGM, NumStory, PGA, g = 386.4):   
    ...
    for i in range(NumHazardLevel):
        ...
        for j in range(NumGM[i]):
            ...
            PFADirectory = os.path.join(DynamicDirectory, 'UniDirection_Miranda240GMs', 'EQ_%d'%(j+1), 'NodeAccelerations')
