...
get_pvalues = function(edp, IM, dist, mag){
  contains_sdr = str_detect(edp, 'sdr')
  contains_pfa = str_detect(edp, 'pfa')
  if (contains_sdr){
    lnSDR = as.matrix(log(dat[edp]))
  }
...
fit <- gam(list(log(story_1_sdrX)  ~ s(mag), log_acc ~ I + s(mag)),
